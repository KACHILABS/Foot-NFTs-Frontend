...
// ===== API FUNCTIONS (built directly into component) =====
...
const DashboardScreen: React.FC<DashboardScreenProps> = ({
  ...
  const [banterPosts, setBanterPosts] = useState<any[]>([]);
  ...
  useEffect(() => {
    const loadDashboardData = async () => {
      ...
      try {
        // Load leaderboard
        const leaderboardRes = await api.leaderboard.getTop();
        if (leaderboardRes.success) {
          setLeaderboardData(leaderboardRes.leaderboard || []);
          const userIndex = leaderboardRes.leaderboard?.findIndex((u: any) => u.id === backendUserId);
          if (userIndex !== undefined && userIndex !== -1) {
            setUserRank(userIndex + 1);
          }
        }
        
        // Load highlights for voting
        const highlightsRes = await api.highlights.getAll();
        if (highlightsRes.success) {
          setHighlightsData(highlightsRes.highlights || []);
        }
        
        // Load trivia question
        const triviaRes = await api.trivia.getQuestion();
        if (triviaRes.success) {
          setTriviaQuestion(triviaRes.question);
        }
        
        // Load banter feed for notifications
        const banterRes = await api.banter.getFeed();
        if (banterRes.success) {
          setBanterPosts(banterRes.posts || []);
        }
      } catch (error) {
  ...
  // Update FTC balance when wallet changes
  useEffect(() => {
  ...
  const totalUnread = unreadBanterCount;
  ...
  const handleBanterNotify = (message: string, mentionedClub: string) => {
    ...
    tg?.showPopup?.({
      ...
      if (buttonId === 'reply') setInBanterHall(true);
    });
  ...
  const handleEarnFTC = async (amount: number, reason: string = 'activity') => {
    ...
    if (onUpdateWallet && wallet) {
      ...
      tg?.showPopup?.({
        title: '🎉 FTC Earned!',
        message: `You earned ${amount} FTC for ${reason}!`,
        buttons: [{ id: 'ok', type: 'default', text: 'Awesome!' }]
      });
      
      // Add to notifications
      const earningNotification: BanterNotification = {
        id: Date.now().toString(),
        message: `You earned ${amount} FTC for ${reason}!`,
        mentionedClub: 'all',
        senderName: 'System',
        senderAvatar: 'https://i.pravatar.cc/150?u=system',
        time: new Date().toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' }),
        read: false,
      };
      setBanterNotifications(prev => [earningNotification, ...prev]);
    }
    onRecordActivity();
  ...
  const renderNotifications = () => (
    <div className="flex flex-col gap-4 pb-20 animate-in fade-in duration-300">
      <div className="flex items-center justify-between px-1">
        ...
        {unreadBanterCount > 0 && (
          <button onClick={markAllRead} className="text-[10px] text-green-500 font-black uppercase tracking-widest">Mark all read</button>
        )}
      </div>
      {banterNotifications.length === 0 ? (
        <div className="flex flex-col items-center justify-center py-16 gap-3 text-center">
          ...
          <button onClick={() => { setShowNotifications(false); setInBanterHall(true); }} className="mt-2 bg-green-600 text-black text-[10px] font-black uppercase tracking-widest px-4 py-2 rounded-xl">Go Start Banter</button>
        </div>
      ) : (
        banterNotifications.map(notif => (
          <div key={notif.id} onClick={() => { setBanterNotifications(prev => prev.map(n => n.id === notif.id ? { ...n, read: true } : n)); setShowNotifications(false); setInBanterHall(true); }} className={`bg-darkCard rounded-2xl p-4 flex items-start gap-3 border cursor-pointer active:scale-[0.98] transition-all ${notif.read ? 'border-gray-800' : 'border-green-500/40 shadow-[0_0_12px_rgba(22,163,74,0.1)]'}`}>
            <div className="relative shrink-0"><img src={notif.senderAvatar} className="w-10 h-10 rounded-xl object-cover border border-gray-800" alt="Avatar" /><span className="absolute -bottom-1 -right-1 text-sm">🔥</span></div>
            <div className="flex-1 min-w-0">
              <div className="flex items-center justify-between gap-2 mb-1"><p className="text-[10px] font-black text-white">{notif.senderName}</p><span className="text-[8px] text-gray-500 font-bold shrink-0">{notif.time}</span></div>
              {notif.mentionedClub !== 'all' && <span className="inline-block text-[7px] font-black uppercase bg-orange-950 text-orange-400 px-1.5 py-0.5 rounded-md mb-1">Mentioned {notif.mentionedClub}</span>}
  ...
  return (
    <div className="h-screen flex flex-col overflow-hidden transition-colors duration-300 bg-transparent">
      {showBanterAlert && (<div className="fixed top-4 left-4 right-4 z-[60] animate-in slide-in-from-top-4 fade-in duration-300"><div className="bg-darkCard border border-green-500/40 rounded-2xl p-4 shadow-2xl flex items-start gap-3"><div className="w-10 h-10 bg-green-600/20 rounded-xl flex items-center justify-center shrink-0"><span className="text-xl">🔥</span></div><div className="flex-1 min-w-0"><p className="text-[10px] font-black text-green-500 uppercase tracking-widest mb-0.5">Banter Alert!</p><p className="text-xs text-white font-bold leading-snug line-clamp-2">{showBanterAlert.message}</p>{showBanterAlert.mentionedClub !== 'all' && <p className="text-[9px] text-orange-400 mt-1">Your club {showBanterAlert.mentionedClub} was called out!</p>}</div><div className="flex flex-col gap-1.5 shrink-0"><button onClick={() => { setShowBanterAlert(null); setInBanterHall(true); }} className="bg-green-600 text-black text-[8px] font-black uppercase px-2.5 py-1.5 rounded-lg tracking-widest">Reply</button><button onClick={() => setShowBanterAlert(null)} className="text-gray-500 text-[8px] font-black uppercase px-2.5 py-1.5 rounded-lg tracking-widest">Dismiss</button></div></div></div>)}

      {showWelcomeBonus && (<div className="fixed inset-0 z-50 flex items-center justify-center px-6 bg-black/80 backdrop-blur-sm animate-in fade-in duration-300"><div className="bg-darkCard rounded-2xl p-6 max-w-sm w-full border border-green-500/30 animate-in zoom-in duration-300"><div className="text-center"><div className="w-20 h-20 mx-auto mb-4 bg-green-600 rounded-full flex items-center justify-center shadow-2xl"><svg className="w-10 h-10 text-black" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2.5" d="M12 8c-1.657 0-3 .895-3 2s1.343 2 3 2 3 .895 3 2-1.343 2-3 2m0-8c1.11 0 2.08.402 2.599 1M12 8V7m0 1v8m0 0v1m0-1c-1.11 0-2.08-.402-2.599-1M21 12a9 9 0 11-18 0 9 9 0 0118 0z" /></svg></div><h2 className="text-2xl font-black text-white mb-2">Welcome to the Pitch! 🎉</h2><p className="text-gray-400 text-sm mb-4">You've joined the Founding Council</p><div className="bg-darkDeep rounded-xl p-4 mb-6 border border-green-500/30"><div className="flex items-center justify-center gap-3"><span className="text-2xl">🎁</span><span className="text-xl font-black text-green-500">+50 FTC</span></div><p className="text-xs text-gray-500 mt-1">Onboarding Bonus</p></div><button onClick={handleClaimWelcomeBonus} className="w-full bg-green-600 text-black py-3 rounded-xl font-black hover:bg-green-500 transition-all active:scale-95">Claim Bonus</button></div></div></div>)}

      {showEditProfile && (<div className="fixed inset-0 z-50 flex items-center justify-center px-6 bg-black/80 backdrop-blur-sm animate-in fade-in duration-300"><div className="bg-darkCard rounded-2xl p-6 max-w-sm w-full border border-green-500/30 animate-in zoom-in duration-300"><div className="text-center mb-4"><div className="w-16 h-16 mx-auto mb-3 bg-green-600/20 rounded-full flex items-center justify-center"><svg className="w-8 h-8 text-green-500" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M16 7a4 4 0 11-8 0 4 4 0 018 0zM12 14a7 7 0 00-7 7h14a7 7 0 00-7-7z" /></svg></div><h2 className="text-xl font-black text-white">Edit Profile</h2><p className="text-xs text-gray-400">Update your display name</p></div><div className="mb-6"><label className="block text-sm font-semibold text-gray-300 mb-2">Display Name</label><input type="text" value={editName} onChange={(e) => setEditName(e.target.value)} className="w-full p-3 rounded-xl bg-darkDeep border border-gray-800 text-white focus:ring-2 focus:ring-green-500 outline-none" placeholder="Enter your name" autoFocus /></div><div className="flex gap-3"><button onClick={() => setShowEditProfile(false)} className="flex-1 bg-gray-800 text-white py-3 rounded-xl font-black hover:bg-gray-700 transition-all">Cancel</button><button onClick={handleSaveProfile} className="flex-1 bg-green-600 text-black py-3 rounded-xl font-black hover:bg-green-500 transition-all">Save Changes</button></div></div></div>)}
      ...
      <div className="px-4 pt-4 pb-3 bg-darkCard/80 backdrop-blur-md shadow-sm flex items-center justify-between border-b border-gray-800 sticky top-0 z-30 shrink-0">
        <div className="flex items-center gap-2 flex-1 min-w-0">
          ...
          {(!showNotifications && !showMarketplace) && (<div className="animate-in fade-in duration-300 flex-1 min-w-0"><div className="flex items-center gap-1.5 flex-wrap"><p className="text-[9px] text-gray-400 font-extrabold uppercase tracking-widest leading-none">Fan Status</p><FanRankBadge rank={profile?.fanRank} /></div><p className="font-bold text-white leading-tight truncate max-w-[150px]">{profile?.displayName}</p></div>)}
        </div>
        <div className="flex items-center gap-1 shrink-0">
          <button onClick={() => { tg?.HapticFeedback.selectionChanged(); setShowNotifications(!showNotifications); setShowMarketplace(false); if (!showNotifications) markAllRead(); }} className={`w-9 h-9 flex items-center justify-center rounded-xl relative active:scale-95 transition-all ${showNotifications ? 'bg-green-600 text-black' : 'bg-gray-800/50 text-gray-400'}`}>
            <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M15 17h5l-1.405-1.405A2.032 2.032 0 0118 14.158V11a6.002 6.002 0 00-4-5.659V5a2 2 0 10-4 0v.341C7.67 6.165 6 8.388 6 11v3.159c0 .538-.214 1.055-.595 1.436L4 17h5m6 0v1a3 3 0 11-6 0v-1m6 0H9" /></svg>
            {totalUnread > 0 && !showNotifications && (<span className="absolute -top-1 -right-1 min-w-[16px] h-4 bg-green-500 rounded-full flex items-center justify-center text-[8px] font-black text-black px-1 animate-bounce">{totalUnread > 9 ? '9+' : totalUnread}</span>)}
          </button>
      ...
      <div className="flex-1 overflow-y-auto px-6 py-6 no-scrollbar bg-transparent">
        {showMarketplace ? <MarketplaceScreen onNotify={() => { tg?.HapticFeedback.notificationOccurred('success'); alert("Interest recorded!"); }} onBack={() => setShowMarketplace(false)} /> : showNotifications ? renderNotifications() : <>
          {activeTab === 'home' && renderHome()}
          {activeTab === 'profile' && (
            <div className="animate-in slide-in-from-bottom-4 duration-500 flex flex-col gap-6">
              <Card className="flex flex-col items-center text-center pt-8 pb-8 relative overflow-hidden bg-darkCard border-gray-800"><div className="absolute top-0 inset-x-0 h-1 bg-green-600 opacity-50"></div><img src={profile?.avatar} className="w-24 h-24 rounded-[2rem] border-4 border-gray-800 shadow-xl mb-4 object-cover" /><h2 className="text-2xl font-black text-white leading-tight break-words max-w-full px-2">{profile?.displayName}</h2><div className="mt-1 flex items-center justify-center gap-2"><FanRankBadge rank={profile?.fanRank} size="sm" /><p className="text-green-500 font-bold uppercase text-[10px] tracking-widest">{badgeTitle}</p></div><div className="flex gap-2 mt-8 w-full"><Button variant="outline" className="flex-1 py-3 text-xs" onClick={() => setShowEditProfile(true)}>Edit Profile</Button><Button variant="outline" className="flex-1 py-3 text-xs" onClick={() => tg?.HapticFeedback.selectionChanged()}>Share Card</Button></div></Card>
