...
interface BanterNotification {
  id: string;
  message: string;
  mentionedClub: string;
  senderName: string;
  senderAvatar: string;
  time: string;
  read: boolean;
...
interface DashboardScreenProps {
  ...
  backendUserId?: string | null;  // ← ADD THIS
  onChangeClub: () => void;
...
const DashboardScreen: React.FC<DashboardScreenProps> = ({
  ...
  // Banter notification state
  const [banterNotifications, setBanterNotifications] = useState<BanterNotification[]>([]);
  const [showBanterAlert, setShowBanterAlert] = useState<BanterNotification | null>(null);
  const banterAlertTimer = useRef<ReturnType<typeof setTimeout> | null>(null);
  ...
  const transactions: Transaction[] = [
    { id: 'tx1', type: 'reward', title: 'Onboarding Bonus', amount: 50, isPositive: true, date: 'Jan 24, 14:30', status: 'completed', icon: '🎁' },
    { id: 'tx2', type: 'activity', title: 'Daily Kit Check-in', amount: 10, isPositive: true, date: 'Jan 25, 09:15', status: 'completed', icon: '👕' },
    { id: 'tx3', type: 'activity', title: 'Trivia Master Reward', amount: 15, isPositive: true, date: 'Jan 25, 11:45', status: 'completed', icon: '⚽' },
    { id: 'tx4', type: 'activity', title: 'Banter Hall Bonus', amount: 2, isPositive: true, date: 'Jan 25, 13:20', status: 'completed', icon: '🔥' },
    { id: 'tx5', type: 'deposit', title: 'TON Swap', amount: 120, isPositive: true, date: 'Jan 26, 10:00', status: 'pending', icon: '💎' },
    { id: 'tx6', type: 'purchase', title: 'Fan Badge Mint', amount: 20, isPositive: false, date: 'Jan 26, 15:20', status: 'completed', icon: '🎖️' },
  ];

  const unreadBanterCount = banterNotifications.filter(n => !n.read).length;
  const totalUnread = unreadBanterCount; // extend here if you add other notification types

  // Handle incoming banter — called by BanterHallScreen
  const handleBanterNotify = (message: string, mentionedClub: string) => {
    const userClubName = club?.name || '';
    const isRelevant =
      mentionedClub === 'all' ||
      mentionedClub.toLowerCase() === userClubName.toLowerCase();

    if (!isRelevant) return;

    const notification: BanterNotification = {
      id: Date.now().toString(),
      message,
      mentionedClub,
      senderName: profile?.displayName || 'A fan',
      senderAvatar: profile?.avatar || 'https://i.pravatar.cc/150?u=99',
      time: new Date().toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' }),
      read: false,
    };

    setBanterNotifications(prev => [notification, ...prev]);

    // Push phone-level notification via Telegram Web App
    tg?.HapticFeedback.notificationOccurred('warning');
    tg?.showPopup?.({
      title: '🔥 Banter Alert!',
      message: `Someone dropped banter about ${mentionedClub === 'all' ? 'your club' : mentionedClub}:\n"${message.slice(0, 80)}${message.length > 80 ? '…' : ''}"`,
      buttons: [
        { id: 'reply', type: 'default', text: 'Reply Now' },
        { id: 'dismiss', type: 'cancel', text: 'Dismiss' },
      ],
    }, (buttonId: string) => {
      if (buttonId === 'reply') {
    ...
    // Floating in-app alert (for when popup isn't supported)
    setShowBanterAlert(notification);
    if (banterAlertTimer.current) clearTimeout(banterAlertTimer.current);
    banterAlertTimer.current = setTimeout(() => setShowBanterAlert(null), 5000);
  };

  const markAllRead = () => {
    setBanterNotifications(prev => prev.map(n => ({ ...n, read: true })));
  };
  ...
  const fanLevel = Math.floor((wallet?.balanceFTC || 0) / 10) + 1;
  const badgeTitle = fanLevel > 10 ? 'Ultra Fan' : fanLevel > 5 ? 'Loyal Supporter' : 'New Signing';

  const handleEarnFTC = (amount: number) => {
    tg?.HapticFeedback.notificationOccurred('success');
    if (onUpdateWallet && wallet) {
      onUpdateWallet({ balanceFTC: (wallet.balanceFTC || 0) + amount });
    }
    onRecordActivity();
  };
  ...
  const renderNotifications = () => (
    <div className="flex flex-col gap-4 pb-20 animate-in fade-in duration-300">
      <div className="flex items-center justify-between px-1">
        <p className="text-[10px] text-gray-400 font-extrabold uppercase tracking-[0.2em]">Banter Alerts</p>
        {unreadBanterCount > 0 && (
          <button onClick={markAllRead} className="text-[10px] text-green-500 font-black uppercase tracking-widest">
            Mark all read
          </button>
        )}
      </div>

      {banterNotifications.length === 0 ? (
        <div className="flex flex-col items-center justify-center py-16 gap-3 text-center">
          <span className="text-4xl opacity-30">🔥</span>
          <p className="text-sm font-black text-gray-500">No banter yet</p>
          <p className="text-[10px] text-gray-600 max-w-[180px]">When fans drop banter about your club, you'll see it here.</p>
          <button
            onClick={() => { setShowNotifications(false); setInBanterHall(true); }}
            className="mt-2 bg-green-600 text-black text-[10px] font-black uppercase tracking-widest px-4 py-2 rounded-xl"
          >
            Go Start Banter
          </button>
        </div>
      ) : (
        banterNotifications.map(notif => (
          <div
            key={notif.id}
            onClick={() => {
              setBanterNotifications(prev => prev.map(n => n.id === notif.id ? { ...n, read: true } : n));
              setShowNotifications(false);
              setInBanterHall(true);
            }}
          ...
          >
            <div className="relative shrink-0">
              <img src={notif.senderAvatar} className="w-10 h-10 rounded-xl object-cover border border-gray-800" alt="Avatar" />
              <span className="absolute -bottom-1 -right-1 text-sm">🔥</span>
            </div>
            <div className="flex-1 min-w-0">
              <div className="flex items-center justify-between gap-2 mb-1">
                <p className="text-[10px] font-black text-white">{notif.senderName}</p>
                <span className="text-[8px] text-gray-500 font-bold shrink-0">{notif.time}</span>
              </div>
              {notif.mentionedClub !== 'all' && (
                <span className="inline-block text-[7px] font-black uppercase bg-orange-950 text-orange-400 px-1.5 py-0.5 rounded-md mb-1">
                  Mentioned {notif.mentionedClub}
                </span>
              )}
              <p className="text-xs text-gray-400 leading-relaxed line-clamp-2">{notif.message}</p>
              <p className="text-[9px] text-green-500 font-black mt-1.5 uppercase tracking-widest">Tap to reply →</p>
            </div>
            {!notif.read && (
              <span className="w-2 h-2 bg-green-500 rounded-full shrink-0 mt-1 animate-pulse" />
            )}
          </div>
        ))
      )}
    ...
    <div className="animate-in slide-in-from-bottom-4 duration-500 flex flex-col gap-6 pb-24">
      <Card className="bg-green-600 text-black p-8 relative overflow-hidden flex flex-col items-center text-center shadow-xl border-none">
        ...
        <h2 className="text-5xl font-black mb-6 tracking-tighter">
          {wallet?.balanceFTC || 0} <span className="text-xl opacity-60">FTC</span>
        </h2>
        <div className="w-full bg-black/20 backdrop-blur-md p-4 rounded-2xl border border-white/10 group active:scale-95 transition-transform cursor-pointer" onClick={() => {
          tg?.HapticFeedback.selectionChanged();
          navigator.clipboard.writeText(wallet?.address || '');
        }}>
          <div className="flex justify-between items-center mb-1">
            <span className="text-[8px] font-black uppercase text-white/60">TON Wallet Address</span>
            <span className="text-[8px] font-black uppercase text-black/80 flex items-center gap-1">
              <span className="w-1 h-1 bg-black rounded-full animate-pulse"></span>
              Verified
            </span>
          </div>
          <div className="flex items-center justify-between gap-2">
            <div className="font-mono text-xs truncate opacity-90">{wallet?.address || 'EQA_...8x92'}</div>
      ...
      <div className="grid grid-cols-2 gap-4">
        <div className="relative group">
          <Button disabled variant="secondary" className="py-4 text-[10px] font-black uppercase tracking-widest border border-gray-800 opacity-60 grayscale bg-darkCard">Deposit TON</Button>
          <div className="absolute -top-2 left-1/2 -translate-x-1/2">
            <span className="bg-gray-800 text-gray-500 text-[7px] font-black px-2 py-0.5 rounded-full uppercase tracking-widest border border-gray-700 shadow-sm">Soon</span>
          </div>
        </div>
        <div className="relative group">
          <Button disabled variant="secondary" className="py-4 text-[10px] font-black uppercase tracking-widest border border-gray-800 opacity-60 grayscale bg-darkCard">Swap FTC</Button>
          <div className="absolute -top-2 left-1/2 -translate-x-1/2">
            <span className="bg-gray-800 text-gray-500 text-[7px] font-black px-2 py-0.5 rounded-full uppercase tracking-widest border border-gray-700 shadow-sm">Soon</span>
          </div>
        </div>
      ...
      <div className="flex flex-col gap-4">
        <div className="flex items-center justify-between px-1">
          <p className="text-[10px] text-gray-400 font-extrabold uppercase tracking-[0.2em]">Transaction History</p>
          <button className="text-[10px] text-green-500 font-black uppercase tracking-widest" onClick={() => tg?.HapticFeedback.selectionChanged()}>View All</button>
        </div>
        <div className="flex flex-col gap-3">
          {transactions.map((tx) => (
            <div key={tx.id} className="bg-darkCard rounded-2xl p-4 flex items-center justify-between border border-gray-800 shadow-sm active:scale-[0.98] transition-all cursor-pointer group" onClick={() => tg?.HapticFeedback.impactOccurred('light')}>
              <div className="flex items-center gap-4">
                <div className={`w-11 h-11 rounded-xl flex items-center justify-center text-xl shadow-inner ${tx.type === 'reward' ? 'bg-orange-950/30 text-orange-500' : tx.type === 'deposit' ? 'bg-blue-950/30 text-blue-500' : tx.type === 'purchase' ? 'bg-purple-950/30 text-purple-500' : 'bg-gray-800 text-gray-500'}`}>{tx.icon}</div>
                <div>
                  <p className="text-sm font-black text-white leading-tight">{tx.title}</p>
                  <div className="flex items-center gap-1.5 mt-0.5">
                    <p className="text-[9px] text-gray-400 font-bold uppercase">{tx.date}</p>
                    <span className="w-0.5 h-0.5 bg-gray-700 rounded-full"></span>
                    <p className={`text-[8px] font-black uppercase tracking-tighter ${tx.status === 'completed' ? 'text-green-500' : tx.status === 'pending' ? 'text-orange-500 animate-pulse' : 'text-red-500'}`}>{tx.status}</p>
                  </div>
                </div>
              </div>
              <div className="text-right">
                <p className={`text-sm font-black ${tx.isPositive ? 'text-green-500' : 'text-white'}`}>{tx.isPositive ? '+' : '-'}{tx.amount} <span className="text-[10px] font-bold opacity-60">FTC</span></p>
                <div className="w-12 h-1 bg-gray-900 rounded-full mt-1.5 overflow-hidden">
                  <div className={`h-full ${tx.isPositive ? 'bg-green-500' : 'bg-gray-700'}`} style={{ width: '100%' }}></div>
                </div>
              </div>
            </div>
          ))}
    ...
    <div className="flex flex-col gap-6 animate-in fade-in duration-500">
      <div className="grid grid-cols-2 gap-4">
        <Card className="bg-green-600 text-black border-none p-5 relative overflow-hidden shadow-lg active:scale-95 transition-transform cursor-pointer" onClick={() => { tg?.HapticFeedback.selectionChanged(); navigateTo('wallet'); }}>
          ...
          <p className="text-3xl font-black">{wallet?.balanceFTC || 0} <span className="text-xs font-bold opacity-60">FTC</span></p>
        </Card>
      ...
      <Card className="bg-darkDeep text-white border border-gray-800 p-6 flex items-center justify-between relative overflow-hidden shadow-2xl transition-all">
        ...
        <div className="relative z-10 flex-1">
          <h3 className="text-lg font-black mb-1 leading-tight tracking-tight">Physical × Digital<br />Jersey Pairing</h3>
          <p className="text-[10px] text-gray-400 font-medium max-w-[140px] leading-relaxed">Wear it in real life. Own it on the blockchain.</p>
        </div>
        ...
        <Card className="flex items-center gap-4 relative overflow-hidden active:scale-[0.98] transition-all cursor-pointer bg-darkCard border-gray-800" onClick={() => { tg?.HapticFeedback.selectionChanged(); navigateTo('club'); }}>
          <div className="w-16 h-16 bg-gray-800 rounded-2xl flex items-center justify-center p-2">
            {club?.badge ? (
              ...
              <div className="w-full h-full rounded-full bg-green-600 flex items-center justify-center text-white font-bold text-xl">
                {profile?.favoriteClubName?.charAt(0) || profile?.favoriteClubId?.charAt(0) || 'FC'}
              </div>
            )}
          </div>
          <div className="flex-1">
            <p className="text-xl font-black text-white leading-tight">
              {club?.name || profile?.favoriteClubName || profile?.favoriteClubId}
            </p>
            <div className="flex items-center gap-2 mt-0.5">
              <span className="text-[10px] bg-orange-950 text-orange-400 font-black px-2 py-0.5 rounded-md uppercase">Lvl {fanLevel}</span>
              <FanRankBadge rank={profile?.fanRank} />
            </div>
          </div>
          ...
          {features.map(f => (
            <div key={f.id} onClick={() => handleFeatureClick(f)} className={`bg-darkCard rounded-[2.5rem] p-5 border border-gray-800 shadow-sm flex flex-col items-center gap-2 active:scale-[0.98] transition-all cursor-pointer text-center group ${f.comingSoon ? 'opacity-80' : ''}`}>
              ...
              <div className="mt-auto w-full pt-2">
                {f.comingSoon ? (
                  <div className="bg-gray-800 text-gray-500 rounded-xl py-2 px-3 flex items-center justify-center border border-gray-700"><span className="text-[8px] font-black uppercase tracking-widest">Coming Soon</span></div>
                ) : (
                  <div className="bg-green-600 text-black rounded-xl py-2 px-3 flex items-center justify-center shadow-md font-black"><span className="text-[8px] font-black uppercase tracking-widest">Access Now</span></div>
                )}
              </div>
            </div>
          ))}
  ...
  return (
    <div className="h-screen flex flex-col overflow-hidden transition-colors duration-300 bg-transparent">
      ...
      {/* ── Floating Banter Alert (in-app push) ── */}
      {showBanterAlert && (
        <div className="fixed top-4 left-4 right-4 z-[60] animate-in slide-in-from-top-4 fade-in duration-300">
          <div className="bg-darkCard border border-green-500/40 rounded-2xl p-4 shadow-2xl flex items-start gap-3">
            ...
            <div className="flex-1 min-w-0">
              <p className="text-[10px] font-black text-green-500 uppercase tracking-widest mb-0.5">Banter Alert!</p>
              <p className="text-xs text-white font-bold leading-snug line-clamp-2">{showBanterAlert.message}</p>
              {showBanterAlert.mentionedClub !== 'all' && (
                <p className="text-[9px] text-orange-400 mt-1">Your club {showBanterAlert.mentionedClub} was called out!</p>
              )}
            </div>
            <div className="flex flex-col gap-1.5 shrink-0">
              <button
                onClick={() => { setShowBanterAlert(null); setInBanterHall(true); }}
                className="bg-green-600 text-black text-[8px] font-black uppercase px-2.5 py-1.5 rounded-lg tracking-widest"
              >
                Reply
              </button>
              <button
                onClick={() => setShowBanterAlert(null)}
                className="text-gray-500 text-[8px] font-black uppercase px-2.5 py-1.5 rounded-lg tracking-widest"
              >
                Dismiss
              </button>
            </div>
            ...
            <div className="text-center">
              ...
              <div className="bg-darkDeep rounded-xl p-4 mb-6 border border-green-500/30">
                <div className="flex items-center justify-center gap-3">
                  ...
                  <span className="text-xl font-black text-green-500">+50 FTC</span>
                </div>
                <p className="text-xs text-gray-500 mt-1">Onboarding Bonus</p>
              </div>
              <button onClick={handleClaimWelcomeBonus} className="w-full bg-green-600 text-black py-3 rounded-xl font-black hover:bg-green-500 transition-all active:scale-95">
                Claim Bonus
              </button>
            </div>
          </div>
          ...
          <div className="bg-darkCard rounded-2xl p-6 max-w-sm w-full border border-green-500/30 animate-in zoom-in duration-300">
            ...
            <div className="mb-6">
              ...
              <input
                ...
                className="w-full p-3 rounded-xl bg-darkDeep border border-gray-800 text-white focus:ring-2 focus:ring-green-500 outline-none"
      ...
      <div className="px-4 pt-4 pb-3 bg-darkCard/80 backdrop-blur-md shadow-sm flex items-center justify-between border-b border-gray-800 sticky top-0 z-30 shrink-0">
          ...
          {(!showNotifications && !showMarketplace) && (
            <div className="animate-in fade-in duration-300 flex-1 min-w-0">
              <div className="flex items-center gap-1.5 flex-wrap">
                <p className="text-[9px] text-gray-400 font-extrabold uppercase tracking-widest leading-none">Fan Status</p>
                <FanRankBadge rank={profile?.fanRank} />
              </div>
        ...
        <div className="flex items-center gap-1 shrink-0">
          ...
          <button
            ...
            className={`w-9 h-9 flex items-center justify-center rounded-xl relative active:scale-95 transition-all ${showNotifications ? 'bg-green-600 text-black' : 'bg-gray-800/50 text-gray-400'}`}
          >
            <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M15 17h5l-1.405-1.405A2.032 2.032 0 0118 14.158V11a6.002 6.002 0 00-4-5.659V5a2 2 0 10-4 0v.341C7.67 6.165 6 8.388 6 11v3.159c0 .538-.214 1.055-.595 1.436L4 17h5m6 0v1a3 3 0 11-6 0v-1m6 0H9" />
            </svg>
            {/* Unread count badge */}
            {totalUnread > 0 && !showNotifications && (
              <span className="absolute -top-1 -right-1 min-w-[16px] h-4 bg-green-500 rounded-full flex items-center justify-center text-[8px] font-black text-black px-1 animate-bounce">
                {totalUnread > 9 ? '9+' : totalUnread}
              </span>
            )}
          </button>
          <button
            onClick={() => { tg?.HapticFeedback.selectionChanged(); setShowMarketplace(!showMarketplace); setShowNotifications(false); }}
            className={`w-9 h-9 flex items-center justify-center rounded-xl active:scale-95 transition-all ${showMarketplace ? 'bg-green-600 text-black' : 'bg-gray-800/50 text-gray-400'}`}
          >
            <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24" strokeWidth="2">
              <path strokeLinecap="round" strokeLinejoin="round" d="M9 3v2m6-2v2M4 7h16M5 7l1 12a2 2 0 002 2h8a2 2 0 002-2l1-12M9 11v4m6-4v4" />
            </svg>
          </button>
        </div>
      </div>
      ...
      <div className="flex-1 overflow-y-auto px-6 py-6 no-scrollbar bg-transparent">
        {showMarketplace ? (
          <MarketplaceScreen onNotify={() => { tg?.HapticFeedback.notificationOccurred('success'); alert("Interest recorded!"); }} onBack={() => setShowMarketplace(false)} />
        ) : showNotifications ? (
          renderNotifications()
        ) : (
          <>
              ...
              <div className="animate-in slide-in-from-bottom-4 duration-500 flex flex-col gap-6">
                <Card className="flex flex-col items-center text-center pt-8 pb-8 relative overflow-hidden bg-darkCard border-gray-800">
                  ...
                  <div className="mt-1 flex items-center justify-center gap-2">
                    <FanRankBadge rank={profile?.fanRank} size="sm" />
                    <p className="text-green-500 font-bold uppercase text-[10px] tracking-widest">{badgeTitle}</p>
                  </div>
                  <div className="flex gap-2 mt-8 w-full">
                    <Button variant="outline" className="flex-1 py-3 text-xs" onClick={() => setShowEditProfile(true)}>Edit Profile</Button>
                    <Button variant="outline" className="flex-1 py-3 text-xs" onClick={() => tg?.HapticFeedback.selectionChanged()}>Share Card</Button>
                  </div>
                </Card>
                <div className="flex flex-col gap-3">
                  <p className="text-[10px] text-gray-400 font-extrabold uppercase tracking-[0.2em] px-1">Referral Program</p>
                  <Card className="bg-darkCard border border-gray-800 p-5">
                    <div className="flex items-center justify-between mb-4">
                      <div className="flex items-center gap-2">
                        ...
                        <div>
                          ...
                          <p className="text-[9px] text-gray-500">Earn 5 FTC per referral</p>
                        </div>
                      </div>
                      <div className="text-right">
                        <p className="text-2xl font-black text-green-500">{onboarding.referralCount}</p>
                        <p className="text-[8px] text-gray-500 uppercase tracking-wider">Referrals</p>
                      </div>
                    </div>
                    <div className="bg-darkDeep rounded-xl p-3 mb-4 border border-gray-800">
                      <div className="flex items-center justify-between gap-2">
                        <code className="text-xs font-mono text-green-500 truncate">{onboarding.referralCode}</code>
                        <button onClick={() => { navigator.clipboard.writeText(onboarding.referralCode); tg?.HapticFeedback.selectionChanged(); tg?.showAlert('Referral code copied!'); }} className="text-[10px] font-black text-green-500 hover:text-green-400 transition-colors">Copy</button>
                      </div>
                    </div>
                    <div className="bg-darkDeep rounded-xl p-3 mb-4 border border-gray-800">
                      <div className="flex items-center justify-between gap-2">
                        <code className="text-[10px] font-mono text-gray-400 truncate">t.me/footnfts_app?ref={onboarding.referralCode}</code>
                        <button onClick={() => { const link = `https://t.me/footnfts_app?ref=${onboarding.referralCode}`; navigator.clipboard.writeText(link); tg?.HapticFeedback.selectionChanged(); tg?.showAlert('Referral link copied!'); }} className="text-[10px] font-black text-green-500 hover:text-green-400 transition-colors">Copy Link</button>
                      </div>
                    </div>
                    <div className="grid grid-cols-2 gap-3">
                      <button onClick={() => { const text = `Join me on FOOT NFTs! Use my referral code: ${onboarding.referralCode}`; window.open(`https://twitter.com/intent/tweet?text=${encodeURIComponent(text)}&url=${encodeURIComponent('https://t.me/footnfts_app?ref=' + onboarding.referralCode)}`, '_blank'); }} className="flex items-center justify-center gap-2 py-2.5 rounded-xl bg-blue-500/10 border border-blue-500/30 text-blue-400 text-[10px] font-black uppercase tracking-wider hover:bg-blue-500/20 transition-all">
                    ...
                    <div className="mt-4 pt-3 border-t border-gray-800">
                      <div className="flex justify-between text-[9px] text-gray-500 mb-1">
                        <span>Next reward at 5 referrals</span>
                        <span>{onboarding.referralCount}/5</span>
                      </div>
                      <div className="w-full h-1.5 bg-gray-800 rounded-full overflow-hidden">
                        <div className="h-full bg-green-600 rounded-full transition-all duration-500" style={{ width: `${Math.min((onboarding.referralCount / 5) * 100, 100)}%` }}></div>
                      </div>
                      {onboarding.referralCount >= 5 && <p className="text-[8px] text-green-500 mt-2 text-center font-bold">🎉 Bonus unlocked! +25 FTC</p>}
                    </div>
                  </Card>
                </div>
                <div className="flex flex-col gap-3">
                  <p className="text-[10px] text-gray-400 font-extrabold uppercase tracking-[0.2em] px-1">Social Stats</p>
                  <div className="grid grid-cols-2 gap-4">
                    <Card className="p-4 bg-darkCard border-gray-800"><p className="text-[10px] text-gray-400 uppercase font-bold mb-1">Influence</p><p className="text-xl font-black text-white">{(onboarding.activityCount * 12).toLocaleString()}</p></Card>
                    <Card className="p-4 bg-darkCard border-gray-800"><p className="text-[10px] text-gray-400 uppercase font-bold mb-1">Rank</p><p className="text-xl font-black text-white">#{(12500 - onboarding.activityCount).toLocaleString()}</p></Card>
                  </div>
                </div>
              </div>
            )}
      ...
      <div className="bg-darkCard/80 backdrop-blur-xl border-t border-gray-800 px-4 py-3 flex justify-between items-center z-40 shrink-0 pb-8">
        {[
          ...
          <button key={tab.id} onClick={() => navigateTo(tab.id)} className={`flex flex-col items-center gap-1 transition-all relative px-2 ${activeTab === tab.id ? 'text-green-500' : 'text-gray-600'}`}>
            ...
            <span className={`text-[7px] font-black uppercase tracking-widest transition-opacity ${activeTab === tab.id ? 'opacity-100' : 'opacity-0'}`}>{tab.label}</span>
          </button>
        ))}
      </div>
    </div>
