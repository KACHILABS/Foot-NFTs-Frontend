...
// Web3 fonts ΓÇö ensure this is in your index.html <head>:
...
const BanterHallScreen: React.FC<BanterHallScreenProps> = ({
  ...
  // ΓöÇΓöÇ Lock Telegram viewport so it never resizes/scrolls away ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
  useEffect(() => {
    if (tg) {
      tg.expand();
  ...
  const sendMessage = async () => {
    ...
    try {
      ...
      const finalContent = replyTarget
        ? `[Reply to ${replyTarget.senderName}: "${replyTarget.content.slice(0, 40)}${replyTarget.content.length > 40 ? 'ΓÇª' : ''}"] ${text}`
        : text;
  ...
  const parseReplyFromContent = (content: string): { replyPart: string | null; mainPart: string } => {
    ...
    if (match) {
      return { replyPart: `Γå⌐ ${match[1]}: ${match[2]}`, mainPart: match[3] };
    }
  ...
  // ΓöÇΓöÇ LOADING ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
  if (loading) {
    return (
      <>
        ...
        <div className="bh-root">
          <div className="bh-header">
            ...
            <div className="bh-avatar">≡ƒÆ¼</div>
  ...
  return (
    <>
      ...
      <div className="bh-root">
        ...
        {/* ΓöÇΓöÇ FIXED HEADER ΓöÇΓöÇ */}
        <div className="bh-header">
          <button className="bh-back-btn" onClick={onBack}><ChevronLeft /></button>
          <div className="bh-avatar">≡ƒÆ¼</div>
          <div className="bh-header-info">
            <span className="bh-title">Banter Hall</span>
            <span className="bh-subtitle">≡ƒöÑ Live ┬╖ {messages.length} msgs</span>
          </div>
          <div className="bh-avatar-stack">
            {messages.slice(-3).map((_, i) => (
              <div key={i} className="bh-mini-avatar">≡ƒæñ</div>
            ))}
          </div>
        </div>

        {/* ΓöÇΓöÇ INFO BANNER ΓöÇΓöÇ */}
        <div className="bh-banner">
          <span className="bh-banner-text">
            ≡ƒÆí Use <strong>#banter</strong> + min {MIN_MESSAGE_LENGTH} chars =&nbsp;
            <strong style={{ color: '#4ade80' }}>+2 FTC</strong>
          </span>
        </div>

        {/* ΓöÇΓöÇ SCROLLABLE MESSAGES ΓöÇΓöÇ */}
        <div
        ...
        >
          {messages.length === 0 ? (
            <div className="bh-empty">
              <div className="bh-empty-icon">≡ƒöÑ</div>
                ...
                <div key={msg.id} className={`bh-msg-row ${msg.isMe ? 'me' : 'them'}`}>
                  <div className={`bh-msg-group ${msg.isMe ? 'me' : 'them'}`}>
                    {!msg.isMe && (
                      <div className="bh-sender-avatar-wrap">
                        {msg.senderAvatar ? (
                          ...
                          <div className="bh-sender-avatar">ΓÜ╜</div>
                        )}
                      </div>
                    )}
                    <div className="bh-msg-col">
                      ...
                      {/* Bubble ΓÇö double-tap to reply */}
                      <div
                      ...
                      >
                        ...
                        {mainPart.toLowerCase().includes('#banter') && (
                          <span className="bh-banter-badge">≡ƒöÑ #banter</span>
                        )}
                      </div>
                      ...
                      <div className={`bh-meta ${msg.isMe ? 'me' : ''}`}>
                        ...
                        <button className="bh-reply-btn" onClick={() => handleReply(msg)}>Γå⌐ Reply</button>
                        {!msg.isMe && (
                          <button className="bh-vote-btn" onClick={() => handleVote(msg.id, msg.userId)}>
                            ≡ƒöÑ Vote {msg.votes > 0 && <span className="bh-votes">({msg.votes})</span>}
        ...
        {/* ΓöÇΓöÇ SCROLL TO BOTTOM BUTTON ΓöÇΓöÇ */}
        {showScrollBtn && (
          <button
        ...
        {/* ΓöÇΓöÇ FIXED INPUT AREA ΓöÇΓöÇ */}
        <div className="bh-input-area">
          ...
          {replyTarget && (
            <div className="bh-reply-strip">
              <div className="bh-reply-strip-info">
                <span className="bh-reply-strip-name">Γå⌐ {replyTarget.senderName}</span>
                <span className="bh-reply-strip-content">
                  {replyTarget.content.slice(0, 60)}{replyTarget.content.length > 60 ? 'ΓÇª' : ''}
                </span>
              </div>
              <button className="bh-reply-close" onClick={() => setReplyTarget(null)}>Γ£ò</button>
            </div>
          )}
          ...
          <p className="bh-input-hint">
            ≡ƒÆí Tap <span style={{ color: '#4ade80', fontWeight: 700 }}>#banter</span> for +2 FTC ┬╖ Double-tap or Γå⌐ Reply ┬╖ Vote for +3 FTC
          </p>
        </div>
      </div>
    </>
...
// ΓöÇΓöÇ SVG ICONS ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
const ChevronLeft = () => (
...
// ΓöÇΓöÇ GLOBAL CSS ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
//
...
// Only .bh-messages has overflow-y:auto ΓÇö everything else is locked.
//
const GLOBAL_STYLES = `
  ...
  /* ΓöÇΓöÇ Root: owns the full viewport ΓöÇΓöÇ */
  .bh-root {
    position: fixed;
    inset: 0;
    display: flex;
  ...
  /* ΓöÇΓöÇ Header (never moves) ΓöÇΓöÇ */
  .bh-header {
    flex-shrink: 0;
    display: flex;
    align-items: center;
  ...
  /* ΓöÇΓöÇ Messages ΓÇö the ONLY scrollable element ΓöÇΓöÇ */
  .bh-messages {
    flex: 1;
    overflow-y: auto;
  ...
  /* ΓöÇΓöÇ Reply quote inside bubble ΓöÇΓöÇ */
  .bh-reply-preview {
  ...
  /* ΓöÇΓöÇ Typing indicator ΓöÇΓöÇ */
  .bh-typing {
  ...
  /* ΓöÇΓöÇ Input area (never moves) ΓöÇΓöÇ */
  .bh-input-area {
    flex-shrink: 0;
    background: #111827;
